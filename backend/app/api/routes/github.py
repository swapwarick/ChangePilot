...
@router.get("/user")
async def get_github_user(token: str = Header(..., alias="Authorization")):
    return await github_provider.get_user_profile(token)


@router.get("/repositories")
async def list_user_repositories(query: str | None = None, token: str = Header(..., alias="Authorization")):
    return await github_provider.list_repositories(token, query=query)


@router.get("/repositories/{owner}/{repo}/branches")
async def list_repository_branches(owner: str, repo: str, token: str = Header(..., alias="Authorization")):
...
async def list_repository_commits(
    owner: str, repo: str, branch: str = "main", limit: int = 30, token: str = Header(..., alias="Authorization")
):
...
async def list_repository_pull_requests(owner: str, repo: str, token: str = Header(..., alias="Authorization")):
...
async def compare_repository_commits(
    owner: str, repo: str, base: str, head: str, token: str = Header(..., alias="Authorization")
):
